“I am Marc Doctor, student of the Bachelor’s Degree in Video Games by UPC at CITM. This content is generated for the second year’s subject Project 2, under supervision of lecturer [Ricard Pillosu](https://www.linkedin.com/in/ricardpillosu/?originalSubdomain=es).”
...
All this NPCs can be QuestGivers, if they are narrative facilitators. The QuestGiver, though, they can appear as a separate figure which only serves that purpose. 
Example: 
...
link
EXERCISE PART 1: Identify the main AI types in your game. 
Take into account:
1.1) Classify them in the act react interact model
1.2) How will you make sure they are perceived? 
Extra) For games with quests → Which ones are QuestGivers? 
                                                   → Is there any emergent NPC? 
     
Reminder: this is only a part of the whole exercise :) 
...
Example: ? 
2) The typical enemy that when the player is trying to solve a puzzle, it appears to frustrate them. 
...
Visualized  
Visualized AI refers to any hint given by the game of NPCs actions or behaviour. The most common uses are when the enemy positions are revealed to the player using an symbol in a minimap. A more advanced technique would be to reveal enemies’ traces or pathfinding routes. 
...
Exercise Part 2: Recover Part 1 “react” section of “act react interact” and… 
1) Decide, in your game, which agents are going to be enemies and which opponents, if any.     
2) Which kinds of objective-targeted-AI types will you use?  
...
Common Attacks 
Waves 
These enemies can attack in the form of waves, meaning that the whole group attacks the player at the same time. This could mean that either: 
The long range group, so it attacks while keeping the formation and position. 
The melee group goes towards the player while keeping the formation. Once it gets to the player, it can still maintain the formation, o surround the player, 
...
Take into account: 
Define the basic group structure and formation. 
...
Exercise last part : Time to manage your whole AI system. 
Draw a basic UML (use draw.io) of your game’s HTN.  
